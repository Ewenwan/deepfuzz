...
static u32 update_min_branch = 10000;
static u32 fair_select = 0;      /* @Ljie@ if exe fair select */

static bool add_select = false;
static bool add_br = false;
...
static u8 fuzz_one(char** argv) {
 ...
 if (skip_deterministic){
  ...
  if(add_select && fair_select && UR(100000) < skip_rate*100000*0.05) {
    // if(not_on_tty) {
  ...
  if(!add_br) {
    ...
    min_branch_hits = is_rb_hit_mini(queue_cur->trace_mini);
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -F            - run with balanced seed selection\n"
       "  -D            - run with hybrid seed mutation\n"
       "  -P            - run with power schedule\n"

       "For additional tips, please consult %s/README.\n"
       "See %s/README.md for options marked.\n\n",
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+bq:rsi:o:f:m:t:T:dnCB:S:M:x:QFDP")) > 0)
